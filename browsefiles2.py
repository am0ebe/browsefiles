#!/home/USER/code/py/o-o/bin/python
...
wait_feedback_symb = "🗣️"
important_symb = "‼️"
urgent_symb = "⏲️"
important_and_urgent_symb = "⏲️‼️"
...
def isEditable(file_abs):
	...
	pp(f"file: {file_abs} of type {file_type}")
	if any( True for e in editables if e in file_type and "XML" not in file_type):
		return True
	else:
		pp(f"skipping uneditable: {file_abs[-10]} of type {file_type}")
		return False
...
def init_curses():
	global gui, contents, maxPage, nColor, tokenAttr
	...
	# Map Pygments tokens to curses color attributes
	tokenAttr = {
		Token.Heading: curses.A_BOLD,
		Token.Strong: curses.A_BOLD,
		Token.Emph: curses.A_UNDERLINE,
		Token.Literal: curses.A_REVERSE,
		Token.Name.Tag: curses.color_pair(2),  # Example tag
		Token.Text: curses.A_NORMAL,
		Token.String: curses.color_pair(3),    # Inline code
		Token.Punctuation: curses.A_NORMAL,
		Token.Keyword: curses.A_BOLD,
	}
...
#highlites MD markdown using pygments and prints
def p(msg="", attr=0, add_newline=True):
	global y, x, gui, tokenAttr

	try:
		tokens = lex(msg, MarkdownLexer())

		for tok_type, tok_val in tokens:
			if x >= curses.COLS - 1:
				break  # Avoid overflow

			# Get matching attr or fallback to default
			style = tokenAttr.get(tok_type, attr)
			trimmed_val = tok_val[:curses.COLS - x - 1]
			gui.addstr(y, x, trimmed_val, style)
			x += len(trimmed_val)
	except Exception as e:
		print(e)
		# pass
...
def print_help():
	...
	x=curses.COLS//3
	p(f"1,2|m,3,4		- show {wait_feedback_symb} ,{important_symb} ,{urgent_symb} ,{important_and_urgent_symb}",color(COLOR_THEME))
	x=curses.COLS//3
...
def main(stdscr):
	...
	while True:
		...
		if ch in [ ord('q'), 27 ]: #ESC
			...
			GLOBAL_SEARCH=important_and_urgent_symb
			#and urgent!
